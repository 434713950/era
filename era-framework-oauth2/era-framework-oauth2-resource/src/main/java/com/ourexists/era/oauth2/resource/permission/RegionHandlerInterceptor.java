...
import com.ourexists.era.oauth2.core.AuthConstants;
...
public class RegionHandlerInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        for (String whiteList : authWhiteListProperties.getHeaderCheck()) {
            if (antPathMatcher.match(whiteList, request.getServletPath())) {
                //白名单设置默认的
                ignore = true;
            }
        ...
        if (antPathMatcher.match(AuthConstants.OVERALL_PREFIX, request.getServletPath())) {
            tenantInfo.setTenantId(CommonConstant.SYSTEM_TENANT);
        }
        if (antPathMatcher.match(AuthConstants.LIMIT_PATH, request.getServletPath())) {
